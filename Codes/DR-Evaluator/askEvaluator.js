/**
 * This skill responds to questions pertaining to power consumption of buildings across Penn's campus. It performs calculations based on the various setpoints like lighting, zone temperature and chilled water temperature and predicts the power consumption based on thse setpoint values.
 * It allows you to choose your own custom strategy and also advises the best strategy that gives the lowest power consumption of College hall
...
function onIntent(context, intentRequest, session, callback) {
	...
	else if ("AssistIntent" === intentName) 
	{
        didNotUnderstand(callback);
    }
...
function getWelcomeResponse(callback) 
{
	...
	var speechOutput = "Hello! Welcome to Prashna's D R Evaluator. I can help you to come up with an efficient power solution for buildings across Penn campus. I can either provide you with an efficient strategy or you may also create your custom strategy. You may start with your questions now.";
				...
				case "1":
					...
					getResponseFromServer(1, [strategy1[0], strategy1[1], strategy1[2], strategy2[0], strategy2[1], strategy2[2], strategy3[0], strategy3[1], strategy3[2]], function(response) 
					{
						...
						if(lowestOutput < baseLineConsumption)
						{
							...
							speechOutput += " You have a curtailment of " + (curtailment / 1000).toFixed(2) + " killo watts than the baseline consumption. You may continue with your questions.";
						...
						else
						{
							...
							speechOutput += " You have an increment by " + (curtailment / 1000).toFixed(2) + " killo watts than the baseline consumption. You may continue with your questions.";
						}
					...
					break;
				case "2":
					speechOutput = "Okay. You have to fix or adjust three different setpoint values for your custom strategy: lighting, zone temperature, and chilled water temperature. Tell me the value of lighting.";
					
					repromptText = "Please tell me the value of lighting. Ensure lighting is greater than 50 percent in order to obtain best results during a D R event.";
	...
	if(setPtType == "lighting" || setPtType == "zone temperature" || setPtType == "cold water temperature" || setPtType == "chilled water temperature")
	{
		if(setPtType == "lighting")
		{
			...
			if(setPtValue >= 0 && setPtValue <= 100)
			{
				...
				speechOutput = "Done. Now, tell me the value of zone temperature. Make sure it is between 25 to 29 degree celsius for best results.";
				repromptText = "I did not understand you. Please tell me the value of zone temperature again.";
			}
			else
			{
				speechOutput = "The lighting value you mentioned is out of range. Lighting should be between 50 to 100 percent during a D R event to obtain best results.";
				repromptText = speechOutput;
			}
			connection.query('INSERT INTO `RTES` (ALEXA, URL_ID, INTENT_NUM) VALUES ("'+speechOutput+'",'+ 0 +','+ 1 +')', function(err, rows, fields) 
			{
		...
		else if(setPtType == "zone temperature")
		{
			if(setPtValue >= 25 && setPtValue <= 29)
			{
				...
				speechOutput = "Alright. Tell me the value of chilled water temperature. Ensure it is in between 6.5 to 10 degree celsius.";
				repromptText = "I did not understand you. Please tell me the value of chilled water temperature again.";
			}
			else
			{
				speechOutput = "Zone temperature value you mentioned is out of range. Zone temperature should be between 25 to 29 degree celsius to obtain best results.";
				repromptText = speechOutput;
			}
			connection.query('INSERT INTO `RTES` (ALEXA, URL_ID, INTENT_NUM) VALUES ("'+speechOutput+'",'+ 0 +','+ 1 +')', function(err, rows, fields) 
			{
			...
			if(setPtValue >= 6 && setPtValue <= 10)
			{
				...
				getResponseFromServer(2, [lightingSetPtValue/100, zoneAirTempSetPtValue, chilledWaterTempSetPtValue], function(response) 
				{
					...
					if(answer[0] < baseLineConsumption)
					{
						...
						speechOutput = "The predicted value of the power consumption of the building using your strategy is " + (answer[0] / 1000000).toFixed(2) + " mega watts. You have a curtailment of " + (curtailment / 1000).toFixed(2) + " killo watts than the baseline consumption. You can continue with your questions.";
					}
					else
					{
						curtailment = answer[0] - baseLineConsumption;
						speechOutput = "The predicted value of the power consumption of the building using your strategy is " + (answer[0] / 1000000).toFixed(2) + " mega watts. You have an increment by " + (curtailment / 1000).toFixed(2) + " killo watts than the baseline consumption. You can continue with your questions.";
					}
...
function changeSetPoint(context, intent, session, callback) 
{
	...
	getResponseFromServer(3, [lightingSetPtValue/100, zoneAirTempSetPtValue, chilledWaterTempSetPtValue], function(response) 
	{
		...
		{
			curtailment = baseLineConsumption - answer[0];
			speechOutput = "The predicted value of the power consumption of the building is " + (answer[0] / 1000000).toFixed(2) + " mega watts. You have a curtailment of " + (curtailment / 1000).toFixed(2) + " killo watts than the baseline consumption. You can continue with your questions.";
			// shouldEndSession = false;
			// callback(sessionAttributes, buildSpeechletResponse(intent.name, speechOutput, repromptText, shouldEndSession));
		}
		else
		{
			curtailment = answer[0] - baseLineConsumption;
			speechOutput = "The predicted value of the power consumption of the building is " + answer[0] + " watts. You have an increment by " + curtailment + " watts than the baseline consumption. You can continue with your questions.";
		}
		connection.query('INSERT INTO `RTES` (ALEXA, URL_ID, INTENT_NUM) VALUES ("'+speechOutput+'",'+ parseInt(plot_id) +','+ 0 +')', function(err, rows, fields) 
		{
			console.log(err);
...
function getWeather(context, intent, session, callback)
{
	...
	//console.log("Current Temperature in Sydney is " + temperature + " degree celsius");
}
...
function didNotUnderstand(callback) 
{
	var sessionAttributes = {};
	var cardTitle = "Did not understand";
	var speechOutput = "I could not understand the framing of your sentence. Please try differently";
